...
import {
  ...
  CONSENTS_GET_BY_TOKEN_ENDPOINT,
  MOCK_ACCEPT_TOKEN,
} from "./utils/constants";
import styles from "./termsPage.module.scss";

export default function TermsPageClient(): React.JSX.Element {
  ...
  const [consent, setConsent] = React.useState<ConsentByTokenResponse | null>(
    null
  );
  ...
  const getUserFriendlyErrorMessage = React.useCallback(
        ...
        return (
          <div role="alert" className={styles.alert}>
            <p className={styles.strong}>
              No pudimos mostrar esta versión de TyC.
            </p>
      ...
      >
        {isReadOnly ? (
          <Card className={styles.card}>
            <CardHeader className={styles.cardHeader}>
              ...
              <CardDescription className={styles.description}>
                Versión informativa (solo lectura).
              </CardDescription>
            </CardHeader>
          ...
          <Card className={`${styles.card} ${styles.cardNarrow}`}>
            <CardHeader className={styles.cardHeader}>
              <CardTitle className={styles.title}>
                ¡Listo! Ya registramos tu aceptación.
              </CardTitle>
              <CardDescription className={styles.description}>
            ...
            <CardFooter className={`${styles.footerRow} ${styles.cardFooter}`}>
              <Button asChild size="lg" className={styles.fullWidthButton}>
                <WhatsAppCta
                  ...
                  message={WHATSAPP_MESSAGE_ACCEPT}
                />
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className={`${styles.card} ${styles.cardNarrow}`}>
            ...
            <CardContent className={styles.contentStackSm}>
              <p className={styles.bodyText}>
                Volvé al chat de WhatsApp de ZAGA y solicitá un nuevo link para
                continuar.
              </p>
            </CardContent>
            {consent.status === "ACCEPTED" ? (
          ...
          <Card className={`${styles.card} ${styles.cardWide} ${styles.cardFixed}`}>
            ...
            <CardContent className={styles.contentScrollable}>
              {errorMessage ? (
                <div role="alert" className={styles.alert}>
                  <p className={styles.strong}>
                    No pudimos validar/registrar tu aceptación.
                  </p>
            ...
            <CardFooter className={`${styles.footerBetween} ${styles.cardFooter}`}>
              <Button
                ...
                disabled={
                  status === "loading" ||
                  !consent?.is_valid ||
                  consent?.terms_version !== "2026-01_v1"
                }
